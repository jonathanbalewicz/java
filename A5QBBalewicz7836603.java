...
public class A5QBBalewicz7836603 {
...
class OrderList {
    ...
    public void sort() {
        ...
        for (int i = 0; i < size; i++) {
            lowest = this.get(i).totalPrice();
            lowestIndex = i;
            for (int j = i + 1; j < size; j++) {
                price = this.get(j).totalPrice();
                if (price < lowest) {
                    lowest = price;
                    lowestIndex = j;
                }
            }
            this.swap(i, lowestIndex);
            
        }
    }
    
    //Swap the position of the order at index 1 with index 2
    public void swap(int index1, int index2) {
        OrderNode nextNode1 = head;
        OrderNode nextNode2 = head;
        Order temp;
